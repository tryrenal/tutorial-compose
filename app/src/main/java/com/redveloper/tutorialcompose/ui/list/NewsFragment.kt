...
import com.redveloper.tutorialcompose.databinding.FragmentNewsBinding
import com.redveloper.tutorialcompose.ui.ViewModelFactory
import com.redveloper.tutorialcompose.ui.detail.NewsDetailActivity
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (tabName == TAB_NEWS) {
            viewModel.getHeadlineNews().observe(viewLifecycleOwner) { result ->
                if (result != null) {
                    when (result) {
                        is com.redveloper.tutorialcompose.data.Result.Loading -> {
                            binding?.progressBar?.visibility = View.VISIBLE
                        }
                        is com.redveloper.tutorialcompose.data.Result.Success -> {
                            binding?.progressBar?.visibility = View.GONE
                            val newsData = result.data
                            newsAdapter.submitList(newsData)
                        }
                        is com.redveloper.tutorialcompose.data.Result.Error -> {
                            binding?.progressBar?.visibility = View.GONE
